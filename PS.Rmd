...
```{r}
sum((retail$Profit - mean(retail$Profit))^2)/(length(retail$Profit)-1)
var(retail$Profit)
```
...
```
```{r}
cat <- subset(retail, Sales > sd(retail$Sales))
cat[order(cat$Sales, decreasing = FALSE),]
...
#====Your Solution====
zz <- (174-180)/8
pnorm(zz, lower.tail=TRUE)
```
```{r}
q1 <- (215.25-146.7)/68.56
```


```{r}
pnorm(q1, lower.tail = FALSE)
sd(mtcars$hp)
```
...
z
pnorm(z) - pnorm(0)
z2 <- (146.7 - mean(mtcars$hp)) / sd(mtcars$hp)
pnorm(z) - pnorm(z2)
```
