...
```{r}
crypto_a <- c(10, 6, 6, 10, 5, 11)
crypto_b <- c(1, 12, 19, 3, 9, 4)
mean(crypto_a)
mean(crypto_b)
```

While both crypto have the same central tendency, they have different levels of volatility, as measured by their variance
...
What is the average horsepower of cars in our catalog?
```{r}
...
When there are discrete values for a variable, the statistical mode refers to the value that occurs most frequently. This statistic is rarely used in practice. Say we are surveying the number of times a customer place a booking in our travel booking app over the period of a year and collected the following sample:
```{r}
...
```{r}
sqrt(sum((retail$Profit - mean(retail$Profit))^2)/(length(retail$Profit)-1))
sd(retail$Profit)
```
...
**Quiz 1: Which financial assets has more volatility in their annual price?**  
```{r}
price.coins <- c(1.4, 0.4, 0.8, 1.1, 1.8, 2.2, 2.3, 1.2)
price.oil <- c(1.6, 1.2, 1.9, 0.8, 0.6, 1.5, 2.1, 1.5)
```

What is the standard deviation of oil?
```{r}
# 0.5126
sd(price.oil)
sqrt(var(price.oil))
...
1. Between horsepower (`hp`) and `mpg` miles per gallon, which has a higher variance?
```{r}
sd(mtcars$hp)
sd(mtcars$mpg)
```
...
2. Which category of items in retail dataset has the lowest variance in their sales prices
```{r}
...
The primary measure of volatility used by stock traders and financial analysts is standard deviation, and recall that this metric reflects the average amount of an item's price over a period of time. While the price for our fictional "oil" asset and "coins" asset averaged out to be USD 1.4 over time, which of these two present a higher volatility than the other? 
...
```{r}
sum((price.coins-mean(price.coins))*(price.oil-mean(price.oil)))/(length(price.oil)-1)
